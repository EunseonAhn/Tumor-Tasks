...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/BrangLab/Downloads/avspeechtask-switch_response_key/AVSpeech.py',
...
if balancer ==1 :
    ...
    aud_list_orig = [x for x in range(10,30)] + [x for x in range(0, 10)] +[x for x in range(30,40)] + [x for x in range(20,30)]
    incongV_list_orig = [x for x in range(10,30)] + [x for x in range(0, 10)] +[x for x in range(30,40)] + [x for x in range(20,30)]
# print(aud_list_orig)
...
practInstr = visual.TextStim(win=win, name='practInstr',
    text="First, we will go through 8 practice trials.\n\nUse the buttons to respond. \nThe '1' key corresponds to the first option shown, the '2' key to the second, '3' to the third, and '4' to the final option.\n\nPress any button to continue.",
    font='Arial',
...
for thisPracticeLoop in practiceLoop:
    ...
    for thisComponent in practiceInstructionComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    practiceLoop.addData('polygon.started', polygon.tStartRefresh)
    practiceLoop.addData('polygon.stopped', polygon.tStopRefresh)
    practiceLoop.addData('practInstr.started', practInstr.tStartRefresh)
    practiceLoop.addData('practInstr.stopped', practInstr.tStopRefresh)
    # check responses
    if practInstResp.keys in ['', [], None]:  # No response was made
        practInstResp.keys = None
    practiceLoop.addData('practInstResp.keys',practInstResp.keys)
    if practInstResp.keys != None:  # we had a response
        practiceLoop.addData('practInstResp.rt', practInstResp.rt)
    practiceLoop.addData('practInstResp.started', practInstResp.tStartRefresh)
    practiceLoop.addData('practInstResp.stopped', practInstResp.tStopRefresh)
    ...
    for thisPracticeTrial in practiceTrials:
        ...
        while continueRoutine and routineTimer.getTime() > 0:
            ...
            if PracticeResponse.status == STARTED and not waitOnFlip:
                theseKeys = PracticeResponse.getKeys(keyList=['1', '2', '3', '4'], waitRelease=False)
...
for thisStimTrial in stimTrials:
    ...
    while continueRoutine:
        ...
        if StimResponseKey.status == STARTED and not waitOnFlip:
            theseKeys = StimResponseKey.getKeys(keyList=['1', '2', '3', '4'], waitRelease=False)
