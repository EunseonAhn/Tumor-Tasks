...
let expName = 'AudNaming';  // from the Builder filename that created this script
...
function trialRoutineBegin(trials) {
  return function () {
    ...
    trialComponents.push(stimSound);
...
function trialRoutineEachFrame(trials) {
  return function () {
    ...
    if (t >= ITI && stimSound.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
    ...
    if (t >= (stimSound.getDuration() + stimSound.tStart)     && stimSound.status === PsychoJS.Status.STARTED) {
      stimSound.stop();  // stop the sound (if longer than duration)
      stimSound.status = PsychoJS.Status.FINISHED;
    }
    
    // *text1* updates
    if (t >= ITI && text1.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
